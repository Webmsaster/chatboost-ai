...
export default function Features() {
  ...
  const features = [
    { icon: Globe, key: "websiteIntegration" },
    { icon: Calendar, key: "appointmentBooking" },
    { icon: BarChart3, key: "analytics" },
    { icon: MessageCircle, key: "whatsapp" },
    { icon: Shield, key: "gdpr" },
    { icon: Zap, key: "gpt" },
    { icon: Languages, key: "multilingual" },
    { icon: UserCheck, key: "leadScoring" },
  ] as const;
        ...
        <div className="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((feature, i) => (
            ...
            >
              ...
              <h3 className="mb-2 font-semibold text-white">{t(`${feature.key}.title`)}</h3>
              <p className="text-sm leading-relaxed text-white/40">{t(`${feature.key}.description`)}</p>
            </motion.div>
          ))}
        </div>
      </div>
